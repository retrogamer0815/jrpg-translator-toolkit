...
Targeted startup and overlay stability checks: `pwsh -NoProfile -File tests/test_conditional_regressions.ps1` verifies saved explanation provider/model choices, independent empty-value fallbacks, brush initialization/reuse/release, and stable Windows GDI resource counts across repeated theme changes and repaints. It also rejects the unsupported single-line `if` assignment pattern that caused these regressions. Run `pwsh -NoProfile -File tests/test_explainer_bounds.ps1` for position-saving timers, unchanged/invalid geometry, closing-window races, and retrying failed settings writes. Both suites use synthetic data and private test windows, support a 32-bit runtime through `-AutoHotkey`, and do not read user settings or send API requests.

Run `pwsh -NoProfile -File tests/test_lifecycle_regressions.ps1` for real output-file locks, stale temporary files, live polling timers, completion-marker handling, malformed numeric settings, startup font/bounds validation, and late Study callbacks after window destruction. Output clearing uses its own empty temporary file with bounded retries; a failed read keeps the last valid text and request state until the next successful poll. Invalid numeric settings use defaults without rewriting the configuration, and valid values respect the existing profile/UI limits. Recommendation dialogs skip late opening focus if they were already closed. This suite also supports `-AutoHotkey` for a 32-bit runtime and uses only synthetic data and private windows.
