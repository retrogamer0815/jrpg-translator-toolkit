...
#Warn
#NoTrayIcon
; Translator and Explainer overlay process.
...
HideOutputCaret(*) {
    ...
    ; RichEdit can recreate its caret after activation, selection, or theme work.
    ; A thread can own only one caret, so destroying this GUI thread's caret is
    ; safe even when Windows briefly reports focus on a different child control.
...
FocusOverlaySink(*) {
    global Overlay, FocusSink
    if !(IsSet(Overlay) && Overlay && Overlay.Hwnd
      && IsSet(FocusSink) && FocusSink && FocusSink.Hwnd)
        return

    ; SetFocus can implicitly activate a top-level window. Only redirect focus
    ; when the overlay is already foreground; never claim it from a game.
    if (DllCall("user32\GetForegroundWindow", "ptr") != Overlay.Hwnd)
        return
...
ApplyConfiguredStartupZOrder(*) {
    global Overlay, IsTop
    if !(IsSet(Overlay) && Overlay && Overlay.Hwnd) || IsTop
        return

    ; A newly shown non-topmost window still starts at the front of the normal
    ; Z-order. Keep it running, but place it behind the emulator without focus.
    static HWND_NOTOPMOST := -2, HWND_BOTTOM := 1
    static SWP_NOSIZE := 0x0001, SWP_NOMOVE := 0x0002, SWP_NOACTIVATE := 0x0010
    flags := SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE
    DllCall("user32\SetWindowPos", "ptr", Overlay.Hwnd, "ptr", HWND_NOTOPMOST
        , "int", 0, "int", 0, "int", 0, "int", 0, "uint", flags)
    DllCall("user32\SetWindowPos", "ptr", Overlay.Hwnd, "ptr", HWND_BOTTOM
        , "int", 0, "int", 0, "int", 0, "int", 0, "uint", flags)
}
...
HideOSRim(hwnd) {
    ...
    if (IsWindows10()) {
        ...
        ; NormalizeUnifiedWindowStyle() already ensures INNER_W is 1 here.
...
NormalizeUnifiedWindowStyle() {
    global BOX_BG, BDR_OUT, BDR_IN, OUTER_W, INNER_W
    ; Border controls are intentionally internal now. Keep the whole client area
    ; one color while retaining the 1px Windows 10 resize safety requirement.
    BDR_OUT := BOX_BG
    BDR_IN := BOX_BG
    OUTER_W := 0
    INNER_W := 0
...
; If a matching overlay already exists, reveal it without taking focus and exit.
__overlayDuplicateTitleMode := A_TitleMatchMode
__overlayDuplicateDetectHidden := A_DetectHiddenWindows
SetTitleMatchMode 3 ; 3 = Match the title exactly
DetectHiddenWindows true
__overlayExistingHwnd := WinExist(__WIN_TITLE)
if __overlayExistingHwnd {
    ShowWindowNoActivate(__overlayExistingHwnd)
    SetTitleMatchMode __overlayDuplicateTitleMode ; Restore the old mode before exiting
    DetectHiddenWindows __overlayDuplicateDetectHidden
    ExitApp
}
SetTitleMatchMode __overlayDuplicateTitleMode ; Restore the old mode if window wasn't found
DetectHiddenWindows __overlayDuplicateDetectHidden
...
EndPickHide(*) {
    ...
    if (__HidOther) {
        other := __EXPLAIN_MODE ? "Translator" : "Explainer"
        old := A_TitleMatchMode
        oldPickDhw := A_DetectHiddenWindows
        SetTitleMatchMode 3
        DetectHiddenWindows true
        otherHwnd := WinExist(other)
        if otherHwnd
            try ShowWindowNoActivate(otherHwnd)
        SetTitleMatchMode old
        DetectHiddenWindows oldPickDhw
        __HidOther := false
    }
}
...
; Wheel scrolling is handled below without focusing the RichEdit.
OnMessage(0x0007, HideOutputCaretOnFocus) ; WM_SETFOCUS → keep RichEdit caret hidden
OnMessage(0x0006, RefocusOverlaySink) ; WM_ACTIVATE
OnMessage(0x0018, RefocusOverlaySink) ; WM_SHOWWINDOW
OnMessage(0x0021, OutputMouseActivate) ; WM_MOUSEACTIVATE
OnMessage(0x0084, OutputHitTest) ; WM_NCHITTEST
OnMessage(0x0201, BlockOutputMouse) ; WM_LBUTTONDOWN
OnMessage(0x0202, BlockOutputMouse) ; WM_LBUTTONUP
OnMessage(0x0203, BlockOutputMouse) ; WM_LBUTTONDBLCLK

RectOuter := Overlay.AddText("x0 y0 w0 h0")
RectInner := Overlay.AddText("x0 y0 w0 h0")
RectPanel := Overlay.AddText("x0 y0 w0 h0")
; Invisible focus target so the RichEdit never keeps a visible caret.
FocusSink := Overlay.AddEdit("x-20 y-20 w1 h1 -Tabstop -Border")
...
; Transparent mouse shield: RichEdit stays visible/themed, but never receives clicks.
ClickShield := Overlay.AddText("x0 y0 w0 h0 BackgroundTrans")
...
; --- Debounce theme packets right after drag ends (safe version) ---
    ...
    ; Old profiles and older control panels may still send separate border
    ; settings. Collapse them before painting so they cannot reappear.
    NormalizeUnifiedWindowStyle()
    ApplyTheme()
...
WM_LBUTTONDOWN(wParam, lParam, msg, hwnd) {
    global Overlay, OutputCtl, ClickShield
    if (IsSet(OutputCtl) && OutputCtl && hwnd = OutputCtl.Hwnd) {
        QueueHideOutputCaret()
        QueueFocusOverlaySink()
        return 0
    }
    if (IsSet(ClickShield) && ClickShield && hwnd = ClickShield.Hwnd) {
        QueueHideOutputCaret()
        QueueFocusOverlaySink()
        return 0
    }
...
if (IsSet(StatusCtl) && StatusCtl && lParam = StatusCtl.Hwnd) {
    DllCall("gdi32\SetTextColor", "ptr", wParam, "int", ColorToBGR(TXT_COLOR))
...
ShowOverlayStatus(symbol := "⌛") {
    ...
    try DllCall("user32\RedrawWindow", "ptr", StatusCtl.Hwnd, "ptr", 0, "ptr", 0
        , "uint", 0x0001 | 0x0004 | 0x0100) ; INVALIDATE|ERASE|UPDATENOW
    try DllCall("user32\UpdateWindow", "ptr", StatusCtl.Hwnd)
}

HideOverlayStatus(*) {
    global StatusCtl, OutputCtl
    if !(IsSet(StatusCtl) && StatusCtl)
        return
...
ScrollOutputToTop(*) {
    ...
    SendMessage(EM_SETSEL, 0, 0, OutputCtl.Hwnd)
    SendMessage(EM_SCROLLCARET, 0, 0, OutputCtl.Hwnd)
    SendMessage(WM_VSCROLL, SB_TOP, 0, OutputCtl.Hwnd)
    QueueHideOutputCaret()
}
...
ScrollOutputToBottom(*) {
    ...
    SendMessage(EM_SETSEL, -1, -1, OutputCtl.Hwnd)
    SendMessage(EM_SCROLLCARET, 0, 0, OutputCtl.Hwnd)
    SendMessage(WM_VSCROLL, SB_BOTTOM, 0, OutputCtl.Hwnd)
    QueueHideOutputCaret()
    QueueFocusOverlaySink()
}
...
HandleMouseWheel(wParam, lParam, msg, hwnd) {
    ...
    if (!IsSet(Overlay) || hwnd != Overlay.Hwnd)
        return
    ; Forward wheel input without focusing the RichEdit, which would show a caret.
    QueueHideOutputCaret()
...
ScrollOutputByWheelDelta(delta) {
    global OutputCtl
...
IsTopVisibleJrpgOverlay(overlayHwnd) {
    if (!overlayHwnd)
        return false
    try {
        if !DllCall("user32\IsWindowVisible", "ptr", overlayHwnd, "int")
            return false
    } catch {
        return false
    }
