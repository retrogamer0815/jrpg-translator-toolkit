param(
    ...
    [string]$OutputDirectory = (Join-Path ([IO.Path]::GetTempPath()) ('jrpg-audio-ownership-' + [Guid]::NewGuid().ToString('N')))
)
$ErrorActionPreference = 'Stop'
$repo = Split-Path -Parent $PSScriptRoot
$source = [IO.File]::ReadAllText((Join-Path $repo 'JRPG Translator.ahk'))
$generated = [IO.File]::ReadAllText((Join-Path $PSScriptRoot 'audio_ownership_harness.ahk'))
foreach ($name in @('AudioProcessAlive', 'AudioProcessRelease', 'AudioProcessRecord', 'AudioSetProcess', 'AudioLaunchProcess',
    'AudioStartOwnedProcess', 'AudioCommandMatches', 'AudioOpenVerifiedProcess', 'AudioSessionProcess', 'AudioSessionClear', 'AudioIsRunning', 'StopAudioCore')) {
    $body = [regex]::Match($source, '(?ms)^' + $name + '\([^\r\n]*\)\s*\{.*?^\}').Value
    if (!$body) { throw "Missing production function: $name" }
    if ($body -match '\bProcessClose\(') { throw "Audio termination must never resolve a PID again: $name" }
    $generated += "`r`n" + $body
}
...
$script = Join-Path $output.FullName 'audio-ownership-generated.ahk'
...
if (!$process.WaitForExit(30000)) { $process.Kill(); throw "Audio ownership test timed out: $output" }
$process.WaitForExit()
