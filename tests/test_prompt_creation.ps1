param(
    ...
    [string]$OutputDirectory = (Join-Path ([IO.Path]::GetTempPath()) ('jrpg-prompt-create-' + [Guid]::NewGuid().ToString('N')))
)
$ErrorActionPreference = 'Stop'
$repo = Split-Path -Parent $PSScriptRoot
$source = [IO.File]::ReadAllText((Join-Path $repo 'JRPG Translator.ahk'))
$generated = [IO.File]::ReadAllText((Join-Path $PSScriptRoot 'prompt_creation_harness.ahk'))
foreach ($name in @('NewPromptProfile', 'NewExplainPromptProfile', 'StudyReaderAddNewVersionPrompt',
    'CPBigBoxStartPromptText', 'PromptFilePath', 'ExplainProfilePath')) {
...
$script = Join-Path $output.FullName 'prompt-create-generated.ahk'
...
if (!$process.WaitForExit(30000)) { $process.Kill(); throw "Prompt creation test timed out: $output" }
$process.WaitForExit()
